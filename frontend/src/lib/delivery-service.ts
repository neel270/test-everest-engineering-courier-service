    ...
    while (remainingPackages.length > 0) {
      ...
      if (combos.length === 0) {
        ...
        combos.forEach((combo) => {
          stepDescription += `${combo.pkg1.id} ${combo.pkg1.weight}kg + ${combo.pkg2.id} ${combo.pkg2.weight}kg   →   02 packages ${combo.totalWeight}kg\n`;
        });
      }
      ...
      const vehicleAssignments: Array<{
        vehicleId: number;
        packages: PackageData[];
        deliveryTime: number;
        maxDistance: number;
      ...
      for (const vehicle of readyVehicles) {
        ...
        // For this implementation, we'll assign the heaviest available package(s)
        // that fit within the vehicle capacity
        const shipment = this.findOptimalShipmentForVehicle(remainingPackages, vehicle);
        if (shipment.packages.length > 0) {
          const maxDistance = Math.max(
            ...shipment.packages.map((p) => p.distance)
          );
          const deliveryTime = this.calculateDeliveryTime(
            maxDistance,
            vehicle.maxSpeed
          ...
          // Remove delivered packages from remaining packages
          shipment.packages.forEach((pkg) => {
      ...
      // Add vehicle assignments to step description
      vehicleAssignments.forEach((assignment) => {
        const packageIds = assignment.packages.map((pkg) => pkg.id).join(" + ");
        stepDescription += `Vehicle ${String(assignment.vehicleId).padStart(
          2,
          "0"
        )}          Delivering ${packageIds}         ${DeliveryService.formatTime(
          assignment.deliveryTime
        )}\n`;
        stepDescription += `                    ${assignment.maxDistance}km/${
          vehicles[assignment.vehicleId - 1].maxSpeed
        }km/hr\n\n`;

        // Show individual package deliveries if multiple packages
        if (assignment.packages.length > 1) {
          assignment.packages.forEach((pkg) => {
            const pkgDeliveryTime = pkg.distance / vehicles[assignment.vehicleId - 1].maxSpeed;
            stepDescription += `                    Delivering ${
              pkg.id
            }         ${DeliveryService.formatTime(pkgDeliveryTime)}\n`;
            stepDescription += `                    ${pkg.distance}/${
              vehicles[assignment.vehicleId - 1].maxSpeed
            }km/hr\n\n`;
          });
        }
        ...
        stepDescription += `Vehicle ${String(assignment.vehicleId).padStart(
          2,
          "0"
        )} will be available after (2*${assignment.deliveryTime.toFixed(
          2
        )})  ${(assignment.deliveryTime * 2).toFixed(2)} hrs\n\n`;
      });
